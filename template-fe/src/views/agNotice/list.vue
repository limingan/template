  ...
  <CrudList ref="cp" :api="api" domainName="AgNotice" :perms="perms" :parentRefs="this.$refs" :queryParamEx="queryParam"
    ...
    <template slot-scope="scope" slot="querySlot">
      <el-form-item label="标题">
        <el-input  v-model="scope.queryParam.title"placeholder="输入标题"></el-input>
      </el-form-item>
    ...
    <template slot="tableColumnSlot">
      ...
      <el-table-column align="center" prop="beginTime" label="开始时间" :formatter="dateTimeFormat">false</el-table-column>

      <el-table-column align="center" prop="endTime" label="结束时间" :formatter="dateTimeFormat">false</el-table-column>
        ...
        <template slot-scope="scope">
          <el-row>
            ...
            <el-tooltip class="item" effect="dark" content="修改" placement="top">
              <el-button type="primary" icon="el-icon-edit" circle @click="clickSettingBBS(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="延期" placement="top">
              <el-button type="warning" icon="el-icon-time" circle @click="updateTimeNotice(scope.row)"></el-button>
            </el-tooltip>
  ...
  export default {
    ...
    methods: {
      ...
      updateNotice(){
        ...
        } else if(parseInt(new Date().getTime()) > parseInt(this.notice.endTime.getTime())){
          this.$message({
            title: '失败',
            message: '结束时间应大于当前时间',
            type: 'error',
        ...
        }else if(parseInt(this.startTime.getTime()) >= parseInt(this.notice.endTime.getTime())){
          this.$message({
            title: '失败',
            message: '结束时间应大于开始时间',
            type: 'error',
      ...
      clickSettingBBS(msg) {
        console.log(msg)
        this.$router.push({path: `/person/create/${msg.autoId}`})

      },
      showNotice(msg) {
        let routeData=this.$router.resolve({path: `/person/showNotice/${msg.autoId}`})
        window.open(routeData.href, '_blank');
      },
      updateTimeNotice(msg) {
        this.dialogVisible = true
        this.notice.autoId=msg.autoId
        this.notice.endTime=new Date(msg.endTime)
        this.startTime=new Date(msg.beginTime)
      },
      deleteNotice(msg) {
        this.$refs.cp.del(msg);
